...
// Parameters for struct cpu_data_entry.
...
// Parameters for struct pmap.
extern size_t OFFSET(pmap, ttep);
...
// Parameters for struct vm_map.
extern size_t OFFSET(vm_map, pmap);

// Parameters for struct vm_page.
extern size_t SIZE(vm_page);
extern size_t OFFSET(vm_page, vmp_q_snext);
extern size_t OFFSET(vm_page, vmp_phys_page);

// CpuDataEntries
extern uint64_t ADDRESS(CpuDataEntries);

// cpu_ttep
extern uint64_t ADDRESS(cpu_ttep);

// gPhysBase
extern uint64_t ADDRESS(gPhysBase);

// gVirtBase
extern uint64_t ADDRESS(gVirtBase);

// ptov_table
extern uint64_t ADDRESS(ptov_table);

// vm_page_array_beginning_addr
extern uint64_t ADDRESS(vm_page_array_beginning_addr);

// vm_page_array_ending_addr
extern uint64_t ADDRESS(vm_page_array_ending_addr);

// vm_first_phys_ppnum
extern uint64_t ADDRESS(vm_first_phys_ppnum);

// ppl_page_count
extern uint64_t ADDRESS(ppl_page_count);

// cpm_allocate()
extern uint64_t ADDRESS(cpm_allocate);

// pmap_mark_page_as_ppl_page()
extern uint64_t ADDRESS(pmap_mark_page_as_ppl_page);

// pmap_mark_page_as_kernel_page()
extern uint64_t ADDRESS(pmap_mark_page_as_kernel_page);

// pmap_alloc_page_for_kern()
extern uint64_t ADDRESS(pmap_alloc_page_for_kern);

// pmap_remove_options()
extern uint64_t ADDRESS(pmap_remove_options);

/*
 * ppl_bypass_parameters_init
 *
 * Description:
 * 	Initialize the parameters for the PPL bypass.
 */
bool ppl_bypass_parameters_init(void);
